...
marketWin = readtable('wholeYearWin.csv');
marketPlace = readtable('wholeYearPlace.csv');
...
% clear up the tables (remove any 2 or 3 horse races)
% win market
idx_w = [];
for i = 1:numID_w
...
% Combine the tables with inx=1 into a big table
W = table();
for i = 1:length(idx_w)
    if idx_w(i) == 1
        W = cat(1, W, TableWin{i});
    end
end
P = table();
for i = 1:length(idx_p)
    if idx_p(i) -- 1
        P = cat(1, P, TablePlace{i});
    end
end


% Ensure that the tables are equal length
% Select the columns you want to compare
P_subset = P(:, {'EVENT_DT', 'SELECTION_ID'});
W_subset = W(:, {'EVENT_DT', 'SELECTION_ID'});
% Find the common rows based on EVENT_DT and SELECTION_ID
[~, P_idx, W_idx] = intersect(P_subset, W_subset, 'rows');
...
eventID_w = W_common.EVENT_ID;      eventID_p = P_common.EVENT_ID;
uniqueID_w = unique(eventID_w);     uniqueID_p = unique(eventID_p);
numID_w = length(uniqueID_w);       numID_p = length(uniqueID_p);
...
for i = 1:numID_w    
    TableWin{i} = W_common(eventID_w == uniqueID_w(i), :);
...
for i = 1:numID_p    
    TablePlace{i} = P_common(eventID_p == uniqueID_p(i), :);
...
figure
for z = 1:numID_p
    scatter(betfair_probs{z}, calc_probs{z}, 'x', 'r')
    hold on
